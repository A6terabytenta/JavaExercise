...
function edit(studentId) {
	...
	$.ajax({
		type : 'GET',
		url : host + 'getStudentById/' + studentId,
		success : function(response) {
			...
			dateOfBirth.value = (new Date(response.studentInfo.dateOfBirth).getFullYear()
								+ "-" + (new Date(response.studentInfo.dateOfBirth).getMonth() + 1)
								+ "-" + new Date(response.studentInfo.dateOfBirth).getDate());
		}
	});
}
...
function removeStudent(studentId) {
	...
	if (choose == true) {
		$.ajax({
			type : 'DELETE',
			url : host + 'deleteStudent/' + studentId,
			success : function(response) {
...
function search(begin, end, currentPage, response) {
	...
	var results = "<tr>"
			...
			+ "<td><input type='text' class='form-control' id='studentCore' /></td>"
			+ "<td><input type='text' class='form-control' id='address' /></td>"
			+ "<td><input type='text' class='form-control' id='averageScore' /></td>"
			+ "<td><input type='text' class='form-control' id='dateOfBirth' /></td>"
			+ "<td><a class='event fa fa-save' onclick='insertStudent()' title='insert'></a> <a class='event fa fa-refresh' onclick='updateStudent()' title='update'></a></td>"
			+ "</tr>";
	for (var i = begin; i < end; i++) {
		results += "<tr>" + "<td>"
				...
				+ "</td>"
				+ "<td>"
				+ response[i].studentCore
				+ "</td>"
				+ "<td>"
				+ response[i].studentInfo.address
				+ "</td>"
				+ "<td>"
				+ response[i].studentInfo.averageScore
				+ "</td>"
				+ "<td>"
				+ new Date(response[i].studentInfo.dateOfBirth).getFullYear()
				+ "-"
				+ (new Date(response[i].studentInfo.dateOfBirth).getMonth() + 1)
				+ "-" + new Date(response[i].studentInfo.dateOfBirth).getDate()
				+ "</td>"
				+ "<td><a class='event fa fa-remove' onclick='removeStudent("
				+ response[i].studentId + ")' "
				+ "title='remove'></a> <a class='event fa fa-edit'"
				+ " onclick='edit(" + response[i].studentId
				+ ")' title='edit'></a></td>" + "</tr>"
...
function sort(value) {
	...
	case "core":
		click++;
		array.sort(function(a, b) {
			return a.studentCore - b.studentCore
		});
		if (click % 2 == 1) {
			array.reverse();
		}
	...
	case "address":
		click++;
		array.sort(function(a, b) {
			return a.studentInfo.address - b.studentInfo.address
		});
		if (click % 2 == 1) {
			array.reverse();
		}
	...
	case "score":
		click++;
		array.sort(function(a, b) {
			return a.studentInfo.averageScore - b.studentInfo.averageScore
		});
		if (click % 2 == 1) {
			array.reverse();
		}
	...
	case "birthday":
		click++;
		array.sort(function(a, b) {
			return a.studentInfo.dateOfBirth - b.studentInfo.dateOfBirth
		});
		if (click % 2 == 1) {
			array.reverse();
		}
				...
				success : function(response) {
					...
					for (var i = begin; i < end; i++) {
						results += "<tr>" + "<td>"
								...
								+ "</td>"
								+ "<td>"
								+ new Date(response[i].studentInfo.dateOfBirth)
										.getFullYear()
								+ "-"
								+ (new Date(response[i].studentInfo.dateOfBirth)
										.getMonth() + 1)
